...
class Permute(nn.Module):
    """Permute tensor dims in forward, then make it contiguous."""
...
class CNN(nn.Sequential):
    ...
    New feature:
        channel_last: if True, perform NCHW -> NHWC (0,2,3,1) BEFORE flatten.
    """
    ...
    def __init__(
        ...
        flatten: bool = True,
        channel_last: bool = False,  # ✅ 新增：flatten 前把 channel 放到最后
    ) -> None:
        """Initialize the CNN.
        ...
        Args:
            ...
            channel_last: If True, permute output NCHW->NHWC before flatten.
        """
        super().__init__()
        ...
        # ✅ channel-last 在 flatten 之前做
        self._channel_last = bool(channel_last)
        if self._channel_last:
            layers.append(Permute(0, 2, 3, 1))  # NCHW -> NHWC
        ...
        # 注意：如果 flatten=False 且 channel_last=True，forward 输出是 [B,H,W,C]，
        # 这里仍按原语义记录：output_channels=C, output_dim=(H,W)
...
def _compute_output_dim(
    ...
    if is_max_pool:
        ...
        h = math.floor((h + 2 * 1 - 1 * (3 - 1) - 1) / 2 + 1)
        w = math.floor((w + 2 * 1 - 1 * (3 - 1) - 1) / 2 + 1)
