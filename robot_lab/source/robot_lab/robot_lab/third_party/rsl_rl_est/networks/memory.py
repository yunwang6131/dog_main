...
class Memory(nn.Module):
    ...
    def forward(self, input, dones=None, hidden_states=None):
        batch_mode = dones is not None
        if batch_mode:
            # batch mode: needs saved hidden states
            if hidden_states is None:
                raise ValueError("Hidden states not passed to memory module during policy update")
            input_traj, masks = split_and_pad_trajectories(input, dones)
            out, _ = self.rnn(input_traj, hidden_states)
            ...
            if hidden_states is None:
                ...
                # inference/distillation mode with passed hidden states
                out, self.hidden_states_inference = self.rnn(input.unsqueeze(0), hidden_states)
        return out
        ...
        if dones is None:  # reset all hidden states
            ...
            if hidden_states is None:
                if isinstance(self.hidden_states, tuple):  # tuple in case of LSTM
                    ...
                    self.hidden_states_inference[..., dones == 1, :] = 0.0
            else:
                NotImplementedError(
