...
class OnPolicyRunnerEst:
    ...
    def _construct_algorithm(self, obs: TensorDict):
        ...
        for key in self.alg_cfg['obs_go_next']:
            obs[key + "_next"] = obs[key].clone()
        for key in self.cfg['obs_groups']['normalize']:
            obs[key + "_norm"] = obs[key].clone()
        for key in self.cfg['obs_groups']['privilege_normalize']:
            obs[key + "_norm"] = obs[key].clone()
        # Resolve RND config
        ...
        actor_critic.reset(torch.ones(obs.batch_size[0],device=self.device))
