...
from deploy_mujoco.sim2sim_core import quat_to_rotmat_wxyz
from deploy_mujoco.sim2sim_kivi_core import KiviSim2SimCfg, KiviSim2SimRunner


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dolanga1 KiVi sim2sim runner (MuJoCo viewer).")
    parser.add_argument("--load_model", type=str, required=True, help="Path to MuJoCo scene xml.")
    parser.add_argument("--policy", type=str, required=True, help="Path to exported KiVi policy_full.pt.")
    ...
    parser.add_argument("--depth_fill_value", type=float, default=2.0, help="Constant depth placeholder value.")
...
def _draw_velocity_arrows(viewer, runner: KiviSim2SimRunner, scale: float) -> None:
...
def main() -> None:
    ...
    cfg = KiviSim2SimCfg(
        ...
        depth_fill_value=args.depth_fill_value,
    )
    runner = KiviSim2SimRunner(cfg)
